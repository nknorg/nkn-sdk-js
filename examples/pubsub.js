...
(async function () {
  ...
  await Promise.all([
    new Promise((resolve) => wallet1.subscribe('some-topic', 100, 'my-identifier').then(resolve)),
    new Promise((resolve) => wallet2.subscribe('some-topic', 100, 'my-identifier').then(resolve)),
  ])
